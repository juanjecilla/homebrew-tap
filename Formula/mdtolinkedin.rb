class Mdtolinkedin < Formula
  ...
  # Default to macOS x86_64 so the spec always has a URL in environments
  # where Homebrew cannot detect CPU architecture.
  ...
  def install
    odie "Linux ARM is not supported by upstream release artifacts" if OS.linux? && !Hardware::CPU.intel?
